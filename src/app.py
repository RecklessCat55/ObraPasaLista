...
def cli_init_db():
    """Crea la BD desde cero (instalación limpia)."""
    db = get_db()
...
def cli_upgrade_db():
    """Migra una BD v1.0 existente a v1.1."""
    db = get_db()
...
def get_lim():
    r = get_db().execute('SELECT limite_horas_dia FROM config WHERE id_config=1').fetchone()
...
def get_rango_activo(id_persona, fecha):
    ...
    se = get_db().execute("SELECT id_rango FROM rango WHERE codigo='SIN_ESPECIFICAR'").fetchone()
...
<div class="container-fluid">
  <a class="navbar-brand" href="/"><i class="bi bi-building-fill"></i> ObraPasaLista
    <span class="badge bg-secondary fw-light ms-1" style="font-size:.55rem">v1.1</span>
  </a>
  ...
  <div class="collapse navbar-collapse" id="nb">
    ...
    <ul class="navbar-nav">
      ...
      <li class="nav-item"><a class="nav-link {% block ac %}{% endblock %}" href="/config/"><i class="bi bi-gear"></i> Config</a></li>
...
def empresa_editar(id_):
    ...
    if request.method == 'POST':
        ...
        if not nom: flash('Nombre obligatorio.', 'error')
            ...
            try:
                db.execute('UPDATE empresa SET nombre=?,estado=? WHERE id_empresa=?', [nom,est,id_])
...
<div class="col-md-8">
  <div class="card"><div class="card-body p-0">
  <table class="table table-hover mb-0 table-sm">
    <thead><tr><th>Código</th><th>Nombre</th><th>Descripción</th><th>Asignaciones</th></tr></thead>
    <tbody>
    {% for r in rangos %}
    <tr>
...
def persona_nueva():
    ...
    if request.method == 'POST':
        ...
        if errs:
            ...
            try:
                cur = db.execute('INSERT INTO persona(nombre,apellido1,apellido2,dni,id_empresa,oficio) VALUES(?,?,?,?,?,?)',
                                 [nom,ap1,ap2,dni,ide,ofi])
...
def persona_editar(id_):
    ...
    if request.method == 'POST':
        ...
        if not nom or not ap1 or not dni:
            ...
            try:
                db.execute('UPDATE persona SET nombre=?,apellido1=?,apellido2=?,dni=?,id_empresa=?,oficio=?,estado=? WHERE id_persona=?',
                           [nom,ap1,ap2,dni,ide,ofi,est,id_])
                if new_r:
...
def obra_editar(id_):
    ...
    if request.method == 'POST':
        nom, cod, est = request.form['nombre'].strip(), request.form['codigo'].strip(), request.form['estado']
        try:
            db.execute('UPDATE obra SET nombre=?,codigo=?,estado=? WHERE id_obra=?', [nom,cod,est,id_])
...
def obra_rem_partida(id_obra, id_p):
    get_db().execute('DELETE FROM partida_obra WHERE id_partida=?', [id_p]); get_db().commit()
    flash('Partida eliminada.', 'success'); return redirect(url_for('obra_editar', id_=id_obra))
...
<div class="row g-3">
<div class="col-md-5">
  <div class="card mb-3"><div class="card-header"><h5 class="mb-0">{{ 'Editar' if ob else 'Nueva' }} obra</h5></div>
  <div class="card-body">
  <form method="post">
    <div class="row g-2 mb-2">
    ...
    {% if ob %}
    <div class="mb-3"><label class="form-label small">Estado</label>
      <select name="estado" class="form-select form-select-sm">
        <option value="activa"    {{ 'selected' if ob.estado=='activa' }}>Activa</option>
        <option value="finalizada"{{ 'selected' if ob.estado=='finalizada' }}>Finalizada</option>
      </select></div>
    {% endif %}
    <div class="d-flex gap-2">
      <button type="submit" class="btn btn-opl btn-sm">Guardar</button>
...
<div class="card mb-3">
  <div class="card-header d-flex justify-content-between align-items-center py-2">
    ...
    <button class="btn btn-sm btn-outline-secondary py-0" type="button"
            data-bs-toggle="collapse" data-bs-target="#fmeteo">
...
<table class="table table-sm table-hover mb-0">
  ...
  <tr>
    ...
    <td class="font-monospace small">{{ l.cod_pa or '—' }}</td>
...
def mensual_cerrar(id_m):
    db = get_db()
    m = db.execute('SELECT * FROM mensual WHERE id_mensual=?', [id_m]).fetchone()
    if not m: flash('No encontrado.', 'error'); return redirect(url_for('mensual_sel'))
...
<div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
  ...
  <div class="d-flex gap-2 flex-wrap">
    ...
    <form method="post" action="/mensual/{{ m.id_mensual }}/reabrir"
          onsubmit="return confirm('¿Reabrir? El snapshot actual se eliminará.')">
      <button class="btn btn-outline-warning btn-sm"><i class="bi bi-unlock"></i> Reabrir</button>
    </form>
    {% endif %}
