...
router.post('/:id_d/arrastrar', (req, res) => {
  ...
  const ant = conn
    .prepare('SELECT id_diario FROM diario WHERE id_obra=? AND fecha<? ORDER BY fecha DESC LIMIT 1')
    .get(d.id_obra, d.fecha);
  if (!ant) {
    req.flash('No hay parte anterior.', 'warning');
    return res.redirect(`/diario/${d.id_obra}/${fsDate(d.fecha)}`);
  }
  ...
  if (ok) req.flash(`${ok} línea(s) arrastradas. Revisa y corrige las horas (están a 0).`, 'info');
  else req.flash('Sin líneas nuevas para arrastrar.', 'warning');
  res.redirect(`/diario/${d.id_obra}/${fsDate(d.fecha)}`);
});
